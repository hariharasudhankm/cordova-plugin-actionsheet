...
- (void) show:(CDVInvokedUrlCommand*)command {
  ...
  [self.commandDelegate runInBackground:^{
    ...
    if ([UIAlertController class]) {
      ...
      alertController = [UIAlertController alertControllerWithTitle:[self encodeString:title]
                                                                 message:[self encodeString:subtitle]
                                                          preferredStyle:UIAlertControllerStyleActionSheet];

      if (addDestructiveButtonWithLabel != nil && !destructiveButtonLast) {
        [self addDestructiveButton:addDestructiveButtonWithLabel toAlertController:alertController asLastItem:destructiveButtonLast relativeToButtons:buttons];
      }
...
- (void) hide:(CDVInvokedUrlCommand*)command {
  dispatch_async(dispatch_get_main_queue(), ^{
    if ([UIAlertController class]) {
//   don't dealloc explicitly : prevent crash on iOS 8+
